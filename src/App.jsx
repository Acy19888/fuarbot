...
async function sendEmail(to, contactName, messeName, salesPerson, smtp) {
  if (!smtp?.smtpHost || !smtp?.smtpUser || !smtp?.smtpPass) return false;
  try {
...
export default function App() {
  ...
  const SMTP_PRESETS = [
    ...
    { label: "Anderer Anbieter", host: "", port: "465" },
  ];
  ...
  const saveContact = async (withEmail = true) => {
    ...
    if (withEmail && current.email) {
      const ok = await sendEmail(current.email, current.name, selectedMesse?.name + " " + selectedMesse?.city, user?.displayName || user?.email, smtp);
      if (ok) { contactData.emailSent = true; setEmailSent(true); setTimeout(() => setEmailSent(false), 2500); notify(`Email an ${current.email} gesendet!`); }
      ...
      {view === "settings" && (
        <div style={{ padding: "20px 20px 110px", animation: "fadeIn .3s" }}>
          ...
          <div style={{ ...S.card, padding: 20, marginBottom: 14 }}>
            ...
            <div style={{ marginBottom: 12 }}>
              <label style={S.label}>Email-Adresse (Absender)</label>
              <input type="email" value={smtp.smtpUser} onChange={(e) => setSmtp((s) => ({ ...s, smtpUser: e.target.value, smtpFrom: e.target.value }))} placeholder="deinname@windoform.de" style={S.input} />
            </div>
            ...
            <button onClick={async () => {
              ...
              setSmtpTesting(true);
              const ok = await sendEmail(smtp.smtpUser, "Test", "Test-Messe", user?.displayName || "Test", smtp);
              setSmtpTesting(false);
              if (ok) notify("Test-Email gesendet an " + smtp.smtpUser + "!");
